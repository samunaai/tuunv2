...
def define_workflow(x1, x2, x3, x4, x5, x6, x7, workflow_name, cache_time):
            ...
            templates=[
                ...
                IoArgoprojWorkflowV1alpha1Template(
                    ...
                    steps=[ IoArgoprojWorkflowV1alpha1ParallelSteps( # STEP 1
                                value=[IoArgoprojWorkflowV1alpha1WorkflowStep(
                                    name="beale-hartmann7d-part1",
                                    template="template1")]),
                            IoArgoprojWorkflowV1alpha1ParallelSteps( # STEP 2
                                value=[IoArgoprojWorkflowV1alpha1WorkflowStep(
                                    name="beale-hartmann7d-part2",
                                    template="template2",
                                    arguments=IoArgoprojWorkflowV1alpha1Arguments(
                                        artifacts=[IoArgoprojWorkflowV1alpha1Artifact(
                                            name="artifact1",_from="{{steps.beale-hartmann7d-part1.outputs.artifacts.output1}}")]
                                        )
                                    )]),
                            IoArgoprojWorkflowV1alpha1ParallelSteps( # STEP 3
                                value=[IoArgoprojWorkflowV1alpha1WorkflowStep(
                                    name="beale-hartmann7d-part3",
                                    template="template3",
                                    arguments=IoArgoprojWorkflowV1alpha1Arguments(
                                        artifacts=[IoArgoprojWorkflowV1alpha1Artifact(
                                            name="artifact2",_from="{{steps.beale-hartmann7d-part2.outputs.artifacts.output2}}")]
                                        )
                                    )]), 
                            IoArgoprojWorkflowV1alpha1ParallelSteps( # STEP 4
                                value=[IoArgoprojWorkflowV1alpha1WorkflowStep(
                                    ...
                                    arguments=IoArgoprojWorkflowV1alpha1Arguments(
                                        artifacts=[IoArgoprojWorkflowV1alpha1Artifact(
                                            name="artifact3",_from="{{steps.beale-hartmann7d-part3.outputs.artifacts.output3}}")
                                            ], 
                                        )
                                    )]), 
                    ]
                ...
                IoArgoprojWorkflowV1alpha1Template(
                    ...
                    memoize=IoArgoprojWorkflowV1alpha1Memoize(
                        key="bh7d{0}{1}".format( str(x1).replace('.','x'), str(x2).replace('.','x') ),
                        max_age=cache_time,
                        cache=IoArgoprojWorkflowV1alpha1Cache(
                            config_map=ConfigMapKeySelector(key="bh7d{0}{1}".format( str(x1).replace('.','x'), str(x2).replace('.','x') ), name="my-config1"))   
                    ),
                    container=Container(
                        image='munachisonwadike/beale-hartmann7d-pipeline', 
                        command=['sh', '-c'], 
                ...
                IoArgoprojWorkflowV1alpha1Template(
                    ...
                    memoize=IoArgoprojWorkflowV1alpha1Memoize(
                        key="bh7d{0}{1}{2}".format( str(x3).replace('.','x'), str(x4).replace('.','x'), str(x5).replace('.','x')  ),
                        max_age=cache_time,
                        cache=IoArgoprojWorkflowV1alpha1Cache(
                            config_map=ConfigMapKeySelector(key="bh7d{0}{1}{2}".format( str(x3).replace('.','x'), 
                                str(x4).replace('.','x'), str(x5).replace('.','x')  ), name="my-config2"))   
                    ),
                    container=Container(
                        image='munachisonwadike/beale-hartmann7d-pipeline', 
                        command=['sh', '-c'], 
                        # above line was just for debugging to be sure I can see mounted volume
                        args=["python step2.py {0} {1} {2} /tmp/; ls /tmp/".format(x3, x4, x5)],
                    ),  
                    outputs=IoArgoprojWorkflowV1alpha1Outputs(
                        artifacts=[ 
                            IoArgoprojWorkflowV1alpha1Artifact(
                ...
                IoArgoprojWorkflowV1alpha1Template(
                    ...
                    memoize=IoArgoprojWorkflowV1alpha1Memoize(
                        key="bh7d{0}{1}".format( str(x6).replace('.','x'), str(x7).replace('.','x') ),
                        max_age=cache_time,
                        cache=IoArgoprojWorkflowV1alpha1Cache(
                            config_map=ConfigMapKeySelector(key="bh7d{0}{1}".format( str(x6).replace('.','x'), 
                                str(x7).replace('.','x') ), name="my-config3"))   
                    ),
                    container=Container(
                        image='munachisonwadike/beale-hartmann7d-pipeline', 
                        command=['sh', '-c'], 
                        # above line was just for debugging to be sure I can see mounted volume
                        args=["python step3.py {0} {1} /tmp/; ls /tmp/".format(x6, x7)],
                    ),  
                    outputs=IoArgoprojWorkflowV1alpha1Outputs(
                        artifacts=[ 
                            IoArgoprojWorkflowV1alpha1Artifact(
                ...
                IoArgoprojWorkflowV1alpha1Template(
                    ...
                    container=Container(
                        image='munachisonwadike/beale-hartmann7d-pipeline', 
                        command=['sh', '-c'], 
                        args=["echo 'funcVal:' $(cat /tmp/step3.txt);"], 
                    ),   
                ),
            ]
        )
...
def bh7d_cost(x1, x2, x3, x4, x5, x6, x7):
    return

def submit_bh7d_workflow(params, refresh_window, cost_type=True, cache_time=''):
    ...
    # 1. First Define Workflow Name
    workflow_name = 'beale-hartmann7d-{0}-{1}-{2}-{3}-{4}-{5}-{6}'.format(x1, x2, x3, x4, x5, x6, x7)
    
    # 2. Then Create the Manifest
    manifest = define_workflow(x1, x2, x3, x4, x5, x6, x7, workflow_name, cache_time)
...
if __name__ == '__main__':
    ...
    """
    submit_bh7d_workflow([4.3, 1.5, 0.5, 1, 1, 2, 3], refresh_window=5, cost_type=True, cache_time='5m')
