...
def define_workflow(x1, x2, x3, x4, workflow_name, cache_time):
            ...
            templates=[
                ...
                IoArgoprojWorkflowV1alpha1Template(
                    ...
                    memoize=IoArgoprojWorkflowV1alpha1Memoize(
                        key="b4d{0}{1}".format( str(x1).replace('.','x'), str(x2).replace('.','x') ),
                        max_age=cache_time,
                        cache=IoArgoprojWorkflowV1alpha1Cache(
                            config_map=ConfigMapKeySelector(key="b4d{0}{1}".format( str(x1).replace('.','x'), str(x2).replace('.','x') ), name="my-config1"))   
                    ),
                    container=Container(
                        image='munachisonwadike/branin4d-pipeline', 
                        command=['sh', '-c'], 
                ...
                IoArgoprojWorkflowV1alpha1Template(
                    ...
                    memoize=IoArgoprojWorkflowV1alpha1Memoize(
                        key="b4d{0}{1}".format( str(x3).replace('.','x'), str(x4).replace('.','x') ),
                        max_age=cache_time,
                        cache=IoArgoprojWorkflowV1alpha1Cache(
                            config_map=ConfigMapKeySelector(key="b4d{0}{1}".format( str(x3).replace('.','x'), str(x4).replace('.','x') ), name="my-config2"))   
                    ),
                    container=Container(
                        image='munachisonwadike/branin4d-pipeline', 
                        command=['sh', '-c'], 
...
def b4d_cost(x1, x2, x3, x4):
    return

def submit_b4d_workflow(params, refresh_window, cost_type=True, cache_time=''):
    ...
    x1, x2, x3, x4 = params
    workflow_name = 'branin4d-{0}-{1}-{2}-{3}'.format(x1, x2, x3, x4)
    
    # 2. Then Create the Manifest
    manifest = define_workflow(x1, x2, x3, x4, workflow_name, cache_time)
...
if __name__ == '__main__':
    ...
    """
    submit_b4d_workflow([6, 5, 2, 10], refresh_window=5, cost_type=True, cache_time='5m')
